...
uint64_t fp_counter = 0;
uint64_t branch_instructions = 0;
uint64_t load_instructions = 0;
uint64_t store_instructions = 0;
uint64_t register_int_read_instructions = 0;
uint64_t register_int_write_instructions = 0;
uint64_t register_fp_read_instructions = 0;
uint64_t register_fp_write_instructions = 0;
uint64_t multi_instructions = 0;
uint64_t dcache_read = 0;
uint64_t dcache_write = 0;
uint64_t mem_read = 0;
...
void print_stats() {
  ...
  std::ofstream counter("power_counter.txt");
        counter << "Total number of instructions:        " << retired_instruction << endl;
        counter << "Total simulation cycles:        " << cycle_count << endl;
        counter << "Number of floating point operations:        " << fp_instructions<< endl;
        counter << "Number of integer operations:        " << retired_instruction-fp_instructions-branch_instructions-load_instructions-store_instructions << endl;
        counter << "Number of branch instructions:        " << branch_instructions << endl;
        counter << "Number of load instructions:        " << load_instructions << endl;
        counter << "Number of store instructions:        " << store_instructions << endl;
        counter << "Number of I-cache access:        " << retired_instruction << endl;
        counter << "Number of branch mispredictions:        " << bpred_mispred_count << endl;
        counter << "Number of scheduler access:                " << retired_instruction << endl;
        counter << "Number of register reads for integer operations:        " << register_int_read_instructions<< endl;
        counter << "Number of register reads for fp operations:                " << register_fp_read_instructions << endl;
        counter << "Number of multiply instructions:        " << multi_instructions << endl;
        counter << "Number of data cache reads:        " << dcache_read << endl;
        counter << "Number of data cache access:        " << dcache_write+dcache_read << endl;
 	 counter << "Number of data cache write:        " << dcache_write << endl;
        counter << "Number of memory read:        " << 2*(mem_read+mem_write) << endl;
        counter << "Number of memory write:        " << 2*(mem_write+mem_read) << endl;
        counter << "Number of total tlb access:        " << dcache_read+dcache_write << endl;
        counter << "Noc0 total access:        " << 4*(mem_read+mem_write)<<endl;
        counter << "system.mem.memory_access:        " << (mem_read+mem_write) << endl;
        counter << "Number of register writes for integer operations:        " << register_int_write_instructions << endl;
        counter << "Number of register writes for fp operations:        " << register_fp_write_instructions << endl;
 	counter << "fpcounter:        " << fp_counter << endl;
	counter << "int_counter:        " << int_counter << endl;
	counter << "total:        " << fp_counter+int_counter << endl;
...
#define FE_CONDITIONAL_BRANCH ((FE_latch->op)->cf_type==CF_CBR)
	...
	if(FE_latch->op_valid == true)		//check if inst lies in the FE latch
	{
		if(FE_latch->op->is_fp){
                                        fp_counter++;
 register_fp_read_instructions += (uint64_t)FE_latch->op->num_src;
                                        if(FE_latch->op->dst != -1){
                                                register_fp_write_instructions++;
                                        }
                                       
                                }else{
                                        int_counter++;
     register_int_read_instructions += (uint64_t)FE_latch->op->num_src;
                                        if(FE_latch->op->dst != -1){
                                                register_int_write_instructions++;
                                        }
                                        }
	...
	if(FE_latch->stage_stall==false)	//check if no pipeline stalled
	{
		...
		if(get_op(new_op))		//copy op from trace into the placeholder op
		{
			   ...
			   if(new_op->is_fp){
                                ...
                                if(new_op->cf_type >= CF_BR){
                                        branch_instructions++;
                                }
                                if(new_op->mem_type == MEM_LD){
                                        load_instructions++;
                                }else if(new_op->mem_type == MEM_ST){
                                        store_instructions++;
                                }
                                if(new_op->opcode == OP_IMUL || new_op->opcode == OP_MM){
                                        multi_instructions++;
                                }
