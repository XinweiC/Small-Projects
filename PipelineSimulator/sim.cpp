  ...
  /* you must complete the function */
	...
	if (FE_latch->op_valid==TRUE){
		if(ID_latch->op_valid==FALSE){
			...
			if(FE_latch->op->src[0]>-1 && register_file[FE_latch->op->src[0]].valid==FALSE){
				data_hazard_count++;
				hazard = true;
	        }
			if (FE_latch->op->src[1]>-1 && register_file[FE_latch->op->src[1]].valid==FALSE){
					data_hazard_count++;
				hazard = true;
			}
