...
int   bpred_gshare_access(bpred *b, unsigned int pc){
	int index;
    index= (pc^b->ghr) % (b->pht_entries);
		if(b->pht[index] > BPRED_PHT_CTR_MAX/2){
			return 1; // Taken
		}
		else{
			return 0; // Not-Taken
...
void  bpred_gshare_update(bpred *b, unsigned int pc, 
    ...
    if(resolve_dir==1){
			b->pht[index] = BPRED_SAT_INC(b->pht[index], BPRED_PHT_CTR_MAX);
		}
    else{
			b->pht[index] = BPRED_SAT_DEC(b->pht[index]);
		}
